...
void ClientPutInServer(edict_t* pEntity)
{
	...
	CBaseEntity* pTarget = UTIL_FindEntityByClassname(NULL, "info_intermission");
	if (pTarget != nullptr)
	{
		pPlayer->StartObserver(pTarget->pev->origin, pTarget->pev->angles);
	}
	else
	{
...
void TFClient_JoinTeam(edict_t* pEntity)
{
	...
	{
		CLIENT_PRINTF(player->edict(), print_center, "Invalid team!");
	}
}
...
void TFClient_JoinClass(edict_t* pEntity)
{
	...
	else
	{
		...
		else if (!stricmp("2", p) || !stricmp("soldier", p))
		{
			player->m_iNewClass = CLASS_SOLDIER;
		...
		{
			CLIENT_PRINTF(player->edict(), print_center, "Invalid class! Pick again.");
			return;
		}
